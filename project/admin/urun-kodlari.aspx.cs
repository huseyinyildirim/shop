...
{
    public partial class urun_kodlari : System.Web.UI.Page
    {
        ...
        protected bool KodKontrol(string Kod)
        {
            ...
            {
                int EID = (from p in db.tbl_kodlar
                           where p.kod == Kod
        ...
        {
            if (!KodKontrol(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TextFieldKod.Text.Trim()).ToUpper()))
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_kodlar ta = new tbl_kodlar();
                    ta.kod = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TextFieldKod.Text.Trim()).ToUpper();
                    ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
                    db.AddTotbl_kodlar(ta);
                    db.SaveChanges();
            ...
            {
                Class.Fonksiyonlar.ExtJS.MesajKutusu(Icon.Error, MessageBox.Icon.ERROR, "İşlem başarısız", "Aynı koddan mevcut.<br/>Lütfen farklı bir kod belirtin!");
                return;
            }
        }
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_kodlar
                            where p.id == ID
                            select new
                            {
                                p.id,
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_kodlar ta = (from p in db.tbl_kodlar where p.id == PID select p).FirstOrDefault();
                ta.kod = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TextFieldKod.Text.Trim()).ToUpper();
