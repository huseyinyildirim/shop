...
{
    public partial class kullanicilar_siparis : System.Web.UI.Page
    {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    var SQL1 = (from p in db.tbl_siparisler
                                from k in db.tbl_siparisler_gonderimler
                                where p.kullanici_id == KullaniciID && k.siparis_id == p.id
                                select new
                                {
                                    p.id,
                                    p.no,
                                    p.toplam,
                                    para_birimi = db.tbl_sbt_para_birim.Where(a => a.id == p.para_birimi_id).Select(a => a.simge == null ? a.kisaltma : a.simge).FirstOrDefault(),
                                    odeme_sekli = db.tbl_sbt_odeme_sekli.Where(a => a.id == p.odeme_sekli_id).Select(a => a.ad).FirstOrDefault(),
                                    p.tarih,
                                    kargo = db.tbl_sbt_kargo.Where(a => a.id == k.kargo_id).Select(a => a.ad).FirstOrDefault(),
                                    durum = "asdas"
                                });
                    #endregion
