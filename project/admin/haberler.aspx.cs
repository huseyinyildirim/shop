    ...
    public partial class haberler : System.Web.UI.Page
    {
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL0 = (db.tbl_haberler).AsEnumerable();
        ...
        protected void YeniKayitYap(object sender, DirectEventArgs e)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_haberler ta = new tbl_haberler();
                ...
                ta.ad = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TextFieldBaslik.Text.Trim());
                ta.text = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_HtmlEditorText.Value.ToString().Trim());
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_haberler
                            where p.id == ID
                            select new
                            {
                                p.dil_id,
                ...
                if (SQL2.Any())
                {
                    ...
                    Duzenle_TextFieldBaslik.Text = SQL2.Select(x => x.ad).FirstOrDefault();
                    Duzenle_HtmlEditorText.Value = SQL2.Select(x => x.text).FirstOrDefault();
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_haberler ta = (from p in db.tbl_haberler where p.id == PID select p).FirstOrDefault();
                ta.dil_id = int.Parse(Duzenle_ComboBoxDil.SelectedItem.Value.ToString());
                ta.ad = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TextFieldBaslik.Text.Trim());
                ta.text = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_HtmlEditorText.Value.ToString().Trim());
        ...
        public void DM_OnayKaldir(int ID)
        {
            ...
            else
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_haberler ta = (from p in db.tbl_haberler where p.id == ID select p).FirstOrDefault();
        ...
        {
            Objeler.Tanimlar.Messages M = AdminClass.DosyaGonder(gonderilecek_dosya, Class.Degiskenler.Site.Yollar.Haber);
