...
{
    public partial class iletisimden_gelen_mailler : System.Web.UI.Page
    {
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_iletisimler
                                select new
                                {
                                    p.id,
                                    p.ad_soyad,
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        ...
                        okundu = AdminClass.EvetHayir(p.okundu)
        ...
        public void DM_Oku(int ID)
        {
                ...
                if (RSM.SelectedRows.Count > 0)
                {
                    for (int i = 0; i < RSM.SelectedRows.Count; i++)
                    {
                        using (BaglantiCumlesi db = new BaglantiCumlesi())
                        {
                            ...
                            tbl_iletisimler ta = (from p in db.tbl_iletisimler where p.id == PID select p).FirstOrDefault();
            ...
            else
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_iletisimler ta = (from p in db.tbl_iletisimler where p.id == ID select p).FirstOrDefault();
