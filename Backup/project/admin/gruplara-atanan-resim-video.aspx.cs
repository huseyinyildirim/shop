...
{
    public partial class gruplara_atanan_resim_video : System.Web.UI.Page
    {
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_dosyalar_kategoriler
                                select new
                                {
                                    p.id,
                                    p.dosya_id,
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        ...
                        p.admin_id_ek,
                        p.admin_id_gun,
                        url = db.tbl_dosyalar.Where(k => k.id == p.dosya_id && k.tip == (int)Class.Sabitler.Dosya.Video).Select(x => x.url).FirstOrDefault(),
                        tip = (db.tbl_dosyalar.Where(k => k.id == p.dosya_id).Select(x => x.tip).FirstOrDefault() == 1 ? "VIDEO" : "RESIM"),
                        dosya = db.tbl_dosyalar.Where(k => k.id == p.dosya_id).Select(x => x.ad).FirstOrDefault(),
                        kategori = db.tbl_dosyalar_kategori.Where(k => k.id == p.kategori_id).Select(x => x.ad).FirstOrDefault(),
            ...
            if (RSM.SelectedRows.Count > 0)
            {
                for (int i = 0; i < RSM.SelectedRows.Count; i++)
                {
                    using (BaglantiCumlesi db = new BaglantiCumlesi())
                    {
                        ...
                        tbl_dosyalar_kategoriler ta = (from p in db.tbl_dosyalar_kategoriler where p.id == PID select p).FirstOrDefault();
                        db.DeleteObject(ta);
