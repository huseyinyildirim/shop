...
{
    public partial class kodlar_scriptler : System.Web.UI.Page
    {
        ...
        protected void StoreDoldurMenu(Store StoreAdi)
        {
            ...
            {
                int DilID = (from p in db.tbl_diller
                             where p.ana_dil == true
                             select p.id).FirstOrDefault();

                var SQL1 = (from p in db.tbl_menuler
        ...
        protected bool ScriptKontrol(int MenuID)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                int EID = (from p in db.tbl_scriptler
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_scriptler
                                select new
                                {
                                    p.id,
                                    p.menu_id,
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        ...
                        p.admin_id_gun,
                        p.menu_id,
                        menu = db.tbl_menuler.Where(k => k.id == p.menu_id).Select(x => x.ad).FirstOrDefault(),
                        menu_url = db.tbl_menuler.Where(k => k.id == p.menu_id).Select(x => x.url).FirstOrDefault(),
        ...
        protected void YeniKayitYap(object sender, DirectEventArgs e)
        {
            ...
            {
                Class.Fonksiyonlar.ExtJS.MesajKutusu(Icon.Error, MessageBox.Icon.ERROR, "İşlem başarısız", "Aynı sayfaya daha önce bir script eklenmiş.<br/>Lütfen farklı bir sayfa belirtin!");
                return;
            }
        }
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_scriptler
                            where p.id == ID
                            select new
                            {
                                p.menu_id,
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_scriptler ta = (from p in db.tbl_scriptler where p.id == PID select p).FirstOrDefault();
        ...
        public void DM_OnayKaldir(int ID)
        {
            ...
            else
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_scriptler ta = (from p in db.tbl_scriptler where p.id == ID select p).FirstOrDefault();
