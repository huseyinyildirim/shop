...
{
    public partial class bottom_copyright_imi : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(ComboBoxDil.SelectedItem.Value))
                {
                    ...
                    ComboBoxDil.SelectedItem.Value = "-1";
                }
                #endregion
            }
            #endregion
        ...
        protected bool BottomKontrol(int DilID)
        {
            ...
            {
                int EID = (from p in db.tbl_bottomlar
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL0 = (db.tbl_bottomlar).AsEnumerable();
        ...
        {
            if (!BottomKontrol(int.Parse(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_ComboBoxDil.Value.ToString().Trim()))))
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ...
                    ta.baslik = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TextFieldBaslik.Text.Trim());
            ...
            {
                Class.Fonksiyonlar.ExtJS.MesajKutusu(Icon.Error, MessageBox.Icon.ERROR, "İşlem başarısız", "Aynı dile ait daha önce bir copyright imi eklenmiş.<br/>Lütfen farklı bir dil belirtin!");
                return;
            }
        }
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_bottomlar
                            where p.id == ID
                            select new
                            {
                                p.dil_id,
        ...
        {
            int DilID = int.Parse(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_ComboBoxDil.Value.ToString().Trim()));
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_bottomlar ta = (from p in db.tbl_bottomlar where p.id == PID select p).FirstOrDefault();

                if (ta.dil_id == DilID)
                {
                    ta.baslik = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TextFieldBaslik.Text.Trim());
        ...
        public void DM_OnayKaldir(int ID)
        {
            ...
            else
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_bottomlar ta = (from p in db.tbl_bottomlar where p.id == ID select p).FirstOrDefault();
