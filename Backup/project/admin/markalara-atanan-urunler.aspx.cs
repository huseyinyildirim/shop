...
{
    public partial class markalara_atanan_urunler : System.Web.UI.Page
    {
        ...
        protected void YeniKayitYap(object sender, DirectEventArgs e)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                db.AddTotbl_urunler_markalar(ta);
                db.SaveChanges();
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_urunler_markalar
                            where p.id == ID
                            select new
                            {
                                p.urun_id,
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_urunler_markalar ta = (from p in db.tbl_urunler_markalar where p.id == PID select p).FirstOrDefault();
