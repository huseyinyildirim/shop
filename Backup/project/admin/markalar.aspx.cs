...
namespace SHOP.admin
{
    public partial class markalar : System.Web.UI.Page
    {
        ...
        protected bool MarkaKontrol(string Marka)
        {
            ...
            {
                int EID = (from p in db.tbl_markalar
                           where p.ad == Marka
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        ...
                        p.admin_id_ek,
                        p.admin_id_gun,
                        ziyaret = db.tbl_ziyaretler.Where(k => k.marka_id == p.id).Count(),
        ...
        {
            if (!MarkaKontrol(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TextFieldMarka.Text.Trim()).ToUpper()))
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    ...
                    ta.ad = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TextFieldMarka.Text.Trim()).ToUpper();
                    ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
                    ta.onay = OnayBool;
                    db.AddTotbl_markalar(ta);
                    db.SaveChanges();
            ...
            {
                Class.Fonksiyonlar.ExtJS.MesajKutusu(Icon.Error, MessageBox.Icon.ERROR, "İşlem başarısız", "Aynı markadan mevcut.<br/>Lütfen farklı bir marka belirtin!");
                return;
            }
        }
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_markalar
                            where p.id == ID
                            select new
                            {
                                p.id,
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_markalar ta = (from p in db.tbl_markalar where p.id == PID select p).FirstOrDefault();
                ta.ad = Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TextFieldMarka.Text.Trim()).ToUpper();
                ta.onay = OnayBool;
                ta.admin_id_gun = Class.Fonksiyonlar.Admin.Genel.AdminID();

                if (Duzenle_Marka.Text != Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TextFieldMarka.Text.Trim()).ToUpper())
                {
                    if (!MarkaKontrol(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TextFieldMarka.Text.Trim()).ToUpper()))
                    {
                        db.SaveChanges();
        ...
        public void DM_OnayKaldir(int ID)
        {
            ...
            else
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_markalar ta = (from p in db.tbl_markalar where p.id == ID select p).FirstOrDefault();
