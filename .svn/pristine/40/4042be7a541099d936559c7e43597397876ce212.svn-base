...
{
    public partial class kategorilere_atanan_urunler : System.Web.UI.Page
    {
        ...
        protected void StoreDoldurUrun(Store StoreAdi)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL1 = (from p in db.tbl_urunler
                            where p.onay == true
                            select
                            new
                            {
                                p.id,
                                urun = (p.kod_id != null ? db.tbl_kodlar.Where(t => t.id == p.kod_id).Select(y => y.kod).FirstOrDefault() + " - " + p.ad : p.ad)
                            });
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_urunler_kategoriler
                                select new
                                {
                                    p.id,
                                    p.kategori_id,
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        ...
                        p.admin_id_ek,
                        p.admin_id_gun,
                        kategori_ziyaret = db.tbl_ziyaretler.Where(k => k.kategori_id == p.kategori_id).Count(),
                        ...
                        kategori_dil = db.tbl_kategoriler.Where(k => k.id == p.kategori_id).AsEnumerable().Select(x => AdminClass.Dil(x.dil_id)).FirstOrDefault(),
                        kategori = db.tbl_kategoriler.Where(k => k.id == p.kategori_id).Select(x => x.ad).FirstOrDefault(),
        ...
        protected void YeniKayitYap(object sender, DirectEventArgs e)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                {
                    ta.kategori_id = int.Parse(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Ekle_TreePanelKategoriler.CheckedNodes[0].NodeID));
                }

                ta.urun_id = int.Parse(Ekle_ComboBoxUrun.SelectedItem.Value.ToString());
                ta.admin_id_ek = Class.Fonksiyonlar.Admin.Genel.AdminID();
                db.AddTotbl_urunler_kategoriler(ta);
                db.SaveChanges();
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_urunler_kategoriler
                            where p.id == ID
                            select new
                            {
                                p.urun_id,
                ...
                if (SQL2.Any())
                {
                    ...
                    Duzenle_LabelKategori.Text = db.tbl_kategoriler.Where(k => k.id == Kat_ID).Select(p => p.ad).FirstOrDefault();
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_urunler_kategoriler ta = (from p in db.tbl_urunler_kategoriler where p.id == PID select p).FirstOrDefault();
                ...
                if (Duzenle_TreePanelKategoriler.CheckedNodes != null)
                {
                    ...
                    {
                        ta.kategori_id = int.Parse(Class.Fonksiyonlar.Genel.StringIslemleri(Class.Sabitler.StringIslemleri.StringIslemTipleri.SQLTemizle, Duzenle_TreePanelKategoriler.CheckedNodes[0].NodeID));
                    }
                }
