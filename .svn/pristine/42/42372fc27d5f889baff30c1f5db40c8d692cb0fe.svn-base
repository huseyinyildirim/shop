...
{
    public partial class headerlar_sloganlar : System.Web.UI.Page
    {
        ...
        {
            StoreAdi.DataSource = AdminClass.ResimleriGetir(Class.Degiskenler.Site.Yollar.Header, "H");
            StoreAdi.DataBind();
        }
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL0 = (db.tbl_headerlar).AsEnumerable();
        ...
        protected void GuncellemeYap(object sender, DirectEventArgs e)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                ...
                tbl_headerlar ta = (from p in db.tbl_headerlar where p.id == PID select p).FirstOrDefault();
        ...
        public void DM_OnayKaldir(int ID)
        {
            ...
            else
            {
                using (BaglantiCumlesi db = new BaglantiCumlesi())
                {
                    tbl_headerlar ta = (from p in db.tbl_headerlar where p.id == ID select p).FirstOrDefault();
