...
{
    public partial class sanal_pos_para_birim : System.Web.UI.Page
    {
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_sbt_sanal_pos_para_birim
                                from t in db.tbl_sbt_para_birim
                                where t.id == p.para_birim_id
                                select new
                                {
                                    p.id,
                                    p.sanal_pos_id,
