        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_siparisler
                                select new
                                {
                                    p.id,
                                    p.kullanici_id,
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        p.id,
                        p.kullanici_id,
                        kullanici = db.tbl_kullanicilar.Where(x => x.id == p.kullanici_id).Select(x => x.ad_soyad + " - " + x.mail).FirstOrDefault(),
                        p.no,
                        p.tutar,
                        p.indirim_tutari,
                        p.toplam_tutar,
                        ...
                        p.genel_toplam,
                        p.indirim_komisyon_id,
                        indirim = db.tbl_cpy_indirimler_komisyonlar.Where(x => x.id == p.indirim_komisyon_id).Select(x => x.ad).FirstOrDefault(),
                        p.komisyon_tutari,
                        p.kargo_payi,
                        p.puan_tutari,
                        p.toplam,
                        p.para_birimi_id,
                        para_birimi = db.tbl_sbt_para_birim.Where(x => x.id == p.para_birimi_id).Select(x => (x.simge == null ? x.kisaltma : x.simge)).FirstOrDefault(),
                        p.odeme_sekli_id,
                        odeme_sekli = db.tbl_sbt_odeme_sekli.Where(x => x.id == p.odeme_sekli_id).Select(x => x.ad).FirstOrDefault(),
                        p.kur_tarih,
                        durum = db.tbl_sbt_siparis_durum.Where(k => k.id == db.tbl_siparisler_durumlar.Where(x => x.siparis_id == p.id).OrderByDescending(x => x.id).Select(x => x.siparis_durum_id).FirstOrDefault()).Select(k => k.ad).FirstOrDefault(),
                        p.tarih,
