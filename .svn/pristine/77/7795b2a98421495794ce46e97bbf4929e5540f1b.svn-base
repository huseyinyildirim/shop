...
{
    public partial class etiketlere_atanan_urunler : System.Web.UI.Page
    {
        ...
        protected void StoreDoldurEtiket(Store StoreAdi)
        {
            ...
            {
                var SQL1 = (from p in db.tbl_etiketler
                            where p.onay == true
                            select
                            new
                            {
        ...
        protected bool EtiketKontrol(int EtiketID, int UrunID)
        {
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                int EID = (from p in db.tbl_urunler_etiketler
        ...
        protected void Veriler(int Baslangic, int Bitis, string s, string Kolon, string AscDesc, out IList Sonuc1, out IList Sonuc2)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                try
                {
                    ...
                    #region İlk (Ham) SQL
                    var SQL1 = (from p in db.tbl_urunler_etiketler
                                select new
                                {
                                    p.id,
                                    p.etiket_id,
                    ...
                    var SQL3 = SQL2.Skip(Baslangic).Take(Bitis).Select(p => new
                    {
                        ...
                        p.admin_id_ek,
                        p.admin_id_gun,
                        etiket_ziyaret = db.tbl_ziyaretler.Where(k => k.etiket_id == p.etiket_id).Count(),
                        ...
                        etiket_dil = db.tbl_etiketler.Where(k => k.id == p.etiket_id).AsEnumerable().Select(x => AdminClass.Dil(x.dil_id)).FirstOrDefault(),
                        etiket = db.tbl_etiketler.Where(k => k.id == p.etiket_id).Select(x => x.ad).FirstOrDefault(),
        ...
        public void DM_GuncellemeEkraniGetir(int ID)
        {
            ...
            using (BaglantiCumlesi db = new BaglantiCumlesi())
            {
                var SQL2 = (from p in db.tbl_urunler_etiketler
                            where p.id == ID
                            select new
                            {
                                p.urun_id,
